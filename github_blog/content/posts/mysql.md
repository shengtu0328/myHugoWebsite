---
title: "Mysql"
date: 2023-05-15T11:23:16+08:00
draft: true
---
...
而update、deletel的时候，产生的undo log日志不仅在回滚时需要，mvcc版本访问也需要，不会立即被删除。

做记录

不同事务或相同事务对同一条记录进行修改，会导致该记录的undolog生成一条记录版本链表，链表的头部是最新的旧记录，链表尾部是
最早的日记录。
...
**readView**（具体查询的时候 返回undolog版本中的哪个版本 要根据readview）
